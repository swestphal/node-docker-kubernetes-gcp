...
resource "google_container_cluster" "primary" {
  name               = "node-docker-kubernetes-gcp-k8s"  # cluster name
  ...
  provisioner "local-exec" {
    ...
    # nodejs-demo-319000 | europe-west1-b	 | node-docker-kubernetes-gcp-k8s
    command = "gcloud container clusters get-credentials node-docker-kubernetes-gcp-k8s --zone europe-west1-b --project node-docker-kubernetes-gcp"
  }
  node_config {
    preemptible  = true
    ...
    tags = ["node-docker-kubernetes-gcp-k8s"]
  }
  timeouts {
